...
print_help() {
    cat << EOF
Usage: $0 [Options][ subcommand | iOS version ] remember you need to have 10 gb free. (put your ipsw in the directory ipsw)
iOS 15 - 12 Dualboot tool 

Options:
    --dualboot          Dualboot your device on iOS 15 with iOS 12
    --jail_chimera      It will create partition on disk + 1 because of Chimera's setup.
    --help              Print this help
    --boot              Boot your second iOS installation (iOS 12)
    --dont_createPart   Don't create the partitions if you have already created them
    --no-baseband       Indicate that the device does not have a baseband
    --restorerootfs     Remove partitions of dualboot 
    --fix_preboot       Restore preboot with the prebootBackup
...
_exit_handler() {
    ...
    echo "[*] A failure log has been made."
}
...
# Prevent Finder from complaining
if [ "$os" = 'Darwin' ]; then
...
if ! python3 -c 'import pkgutil; exit(not pkgutil.find_loader("pyimg4"))' 2>/dev/null; then
...
# Re-create work dir if it exists
if [ -e work ]; then
    rm -rf work
    mkdir work
else
...
echo "Dualboot tool | iOS 15 -> iOS 12"
echo "Written by Samuel and most code of palera1n :) Thanks, Nebula and Mineek | Some code and ramdisk from Nathan."
echo ""
...
echo "[*] Detected $(get_device_mode | sed 's/dfu/DFU/') mode device"
...
if[ "$boot" = "1" ]; then
    ...
    # Add Trustcache for Chimera/iOS 12 execution if it exists
    if[ -f "boot/${deviceid}/trustcache.img4" ]; then
        "$dir"/irecovery -f "boot/${deviceid}/trustcache.img4"
        sleep 1
        "$dir"/irecovery -c "trustcache"
        sleep 1
    fi
...
if [ "$os" = 'Darwin' ]; then
    if [ ! -f "ipsw/out.dmg" ]; then
        rootfs_path=$(/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."OS"."Info"."Path" xml1 -o - work/BuildManifest.plist | grep '<string>' | cut -d\> -f2 | cut -d\< -f1 | head -1)
        asr -source "$extractedIpsw/$rootfs_path" -target ipsw/out.dmg --embed -erase -noprompt --chunkchecksum --puppetstrings
    fi
fi

if [ "$os" == "Linux" ]; then
    echo "If you have linux, you must look for a mac and execute 'asr -source xxx.xxxxx.xxx.dmg -target out.dmg --embed -erase -noprompt --chunkchecksum --puppetstrings' in order so apfs_invert can mount the root dmg. Linux does not have it, so you must use a mac and copy the result file (out.dmg) to linux on the directory ipsw/out.dmg"
fi
...
if [ ! -f blobs/"$deviceid"-"$version".shsh2 ]; then
    ...
    if[ "$jail_chimera" = "1" ]; then
        disk=$(($disk + 1))
        echo "Disk bumped for Chimera: $disk"
    fi
    
    dataB=$(($disk + 1))
    prebootB=$(($dataB + 1))
    echo "Base Disk: $disk | Data Disk: $dataB | Preboot Disk: $prebootB"
    ...
    if[ "$dualboot" = "1" ]; then
        if [ -z "$dont_createPart" ]; then
            ...
            if [ "$os" = "Darwin" ]; then
                remote_cmd "/sbin/mount_apfs /dev/disk0s1s${disk} /mnt8/"
                echo "[*] Copying filesystem so hang on, that could take 20 minutes because it's through ssh"
        ...
        echo "[*] Copying files to work dir"
        ...
        echo "[*] Patching boot files for iOS 12..."
        ...
        # Kernelcache (iOS 12 args + kerneldiff binary + Kernel64Patcher2)
        "$dir"/img4 -i work/"$(awk "/""${model}""/{x=1}x&&/kernelcache.release/{print;exit}" work/BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1)" -o work/kernel.raw
        "$dir"/Kernel64Patcher2 work/kernel.raw work/kernel.patched -u 12 --skip-sks --skip-acm --skip-amfi
        "$dir"/kerneldiff work/kernel.raw work/kernel.patched work/kernel.bpatch
        "$dir"/img4 -i work/"$(awk "/""${model}""/{x=1}x&&/kernelcache.release/{print;exit}" work/BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1)" -o work/kernelcache.img4 -M work/IM4M -P work/kernel.bpatch -T rkrn -J
        ...
        if [ "$os" = "Linux" ]; then
            echo "devicetree patcher could fall down or not work on linux, however you can use darling to execute it."
        fi
